...
# ***This code is essentially the same as v745_sco_solve_temp_tau_v1 except the
#    'non-stellar-subtracted' data is fitted to dusty models + model stellar
#     continuum'***
...
#
# The program takes the Spitzer spectra and searches through a grid of models to 
# find the one giving the lowest reduced chisq. The models file names contain the 
# parameters temp and tau which are extracted and stored alongide the value of chisq. 
#
...
# spectra. The model of the stellar continuum derived by V745_SCO_DATA_PREP is added
# to each model to produce combined models of dust and stellar emission. The CALC_CHISQ
# function takes this flux and scales it to the flux in the Spitzer spectra (in Jy), 
# returning chisq.
#
# INPUTS: 
...
# DATA_FILEPATH (str) : Data file name. This is the output txt file created by 
#     V745_SCO_DATA_PREP containing wavelength 'stellar-subtracted data  (i.e. dust 
#     emission only, data, and model of the stellar continuum.
#
# OUTPUTS:
...
def read_data_file(data_filepath):
    """
    Read data file with wavelength, flux, and model flux
    Returns wavelength, flux, and model flux as arrays
    """
    wavelengths = []
    fluxes = []
    model_fluxes = []
    ...
    with open(data_filepath, 'r') as f:
        ...
        for line in f:
            values = line.strip().split()
            if len(values) >= 3:  # We need at least 3 columns
                wavelength = float(values[0])
                flux = float(values[2])
                # MODEL_FLUX is the model stellar continuum
                model_flux = float(values[3])
               
                wavelengths.append(wavelength)
                fluxes.append(flux)
                model_fluxes.append(model_flux)

    return np.array(wavelengths), np.array(fluxes), np.array(model_fluxes)
...
def process_all_models(model_directory, data_filepath, error_col=None):
    """Process model and calculate chisq against data"""
    results = {}
    
    # Read data with all three columns
    data_wavelength, data_flux, data_model_flux = read_data_file(data_filepath)
    
    # Estimate errors as sqrt(flux) if not provided
    if error_col is None:
        data_error = np.sqrt(np.abs(data_flux))
        data_error[data_flux <= 0] = 1.0
    ...
    print(f"Read data file with {len(data_wavelength)} data points")
        ...
        if temp is not None and tau is not None:
            try:
                ...
                valid_mask, interp_model_flux = interpolate_model_to_data(
                    data_wavelength, model_wavelength, model_flux)
                ...
                valid_data_model_flux = data_model_flux[valid_mask]
                
                # Add the third column (model flux) to the interpolated model flux
                adjusted_model_flux = valid_model_flux + valid_data_model_flux
                
                # Calculate chisq for the model
                chisq, scaled_model_flux = calc_chisq(valid_data_flux, valid_data_error, adjusted_model_flux)
    ...
    # Read the full data file for plotting (same as above since we're not filtering by wavelength)
    full_data_wavelength, full_data_flux, _ = read_data_file(data_filepath)
...
def plot_best_fit(results, data_wavelength, data_flux, data_error, 
                 full_data_wavelength, full_data_flux, output_dir='results'):
...
def save_results_to_file(results, output_dir='results'):
    ...
    output_file = os.path.join(output_dir, f'chisq_results.txt')
    
    with open(output_file, 'w') as f:
        f.write(f"# Chi-square results\n")
...
if __name__ == "__main__":
    ...
    # Path to data file - use the provided test.dat file
